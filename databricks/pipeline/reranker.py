"""
Reranker for Feeds.ai pipeline.
Logic is identical to the original; data source changed from data_loader (JSON)
to entity_store (in-memory from MLflow artifact).
"""
...
def _ensure_lookups():
    ...
    if _entity_keywords is None:
        ...
        _entity_franchises = {e["entity_id"]: e.get("franchise") for e in entities}
        _entity_composed   = {
...
def rerank(candidates, positive_entities, nlu_output, query_mode, top_k=TOP_K_RESULTS):
    ...
    pos_ids        = {e["entity_id"] for e in positive_entities}
    pos_franchises = {e.get("franchise") for e in positive_entities if e.get("franchise")}
    pos_verticals  = {e["vertical"] for e in positive_entities}
    ...
    for c in results:
        ...
        c["shared_keywords"] = sorted(anchor_kw_set & set(_get_kw(eid)))

    results.sort(key=lambda x: x["final_score"], reverse=True)

    # Per-vertical or single-vertical output
