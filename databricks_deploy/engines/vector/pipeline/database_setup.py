...
def create_users_table():
    ...
    users = [
        ("feeds.ai@hiteshi.com", "Feeds@Discover2026"),
        ("admin@hiteshi.com", "Admin@Hiteshi2026"),
    ]
    for email, password in users:
        pw_hash = hashlib.sha256((salt + password).encode()).hexdigest()
        cur.execute(
            "INSERT INTO app_users (email, password_hash) VALUES (%s, %s) ON CONFLICT (email) DO NOTHING;",
            (email, pw_hash),
        )
    conn.commit()
    cur.close()
