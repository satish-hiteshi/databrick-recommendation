...
def setup_qdrant():
    ...
    if os.getenv("VECTOR_BACKEND", "").lower() == "databricks":
        ...
        print(f"VECTOR_BACKEND=databricks → dense ANN via Databricks Vector Search; "
              f"BM25 index built over {len(_bm25_entities)} entities (Qdrant skipped).")
        return len(_bm25_entities)
...
# Local dev (default) keeps the in-memory Qdrant `vector_search` above. The Databricks Vector App sets
# VECTOR_BACKEND=databricks; the real Vector Search implementation lives in
# databricks_deploy/vector_search/vs_store.py and is loaded BY FILE PATH (not `import databricks...`)
# so it never clashes with the Databricks SDK's own `databricks` import namespace. Same signature and
# return shape, so retrieval.py is unaffected. keyword_search (BM25) stays in-process above.
if os.getenv("VECTOR_BACKEND", "").lower() == "databricks":
    try:                                                # bundle: vs_store already on sys.path
        from vs_store import vector_search  # noqa: F811  (overrides the Qdrant dense ANN)
    except ImportError:                                 # local repo: load from databricks_deploy/vector_search
        import sys as _sys
        _vs_dir = os.path.join(
            os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))),
            "databricks_deploy", "vector_search")
        if _vs_dir not in _sys.path:
            _sys.path.insert(0, _vs_dir)
        from vs_store import vector_search  # noqa: F811,E402
