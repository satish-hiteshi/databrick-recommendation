...
class CandidateMoment:
    # identity + link (the two id rules: moment_id is identity, property_id is the link — never conflated)
    moment_id: int
    property_id: int
    # property context (from the :Entity the moment hangs off)
    entity_id: str                     # prefixed property id, e.g. 'Movie:1100083'
...
class CandidatePool:
    ...
    user_id: int
    followed_property_ids: List[int]               # the hard boundary for the whole stream
