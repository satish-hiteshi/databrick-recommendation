"""exclude_followed — REUSE endpoint_3 CsvFollowSource, behind a try/fallback that degrades to no-exclusion.

UC7 (onboarding) is pre-auth: null user_id → no exclusion (handled in request.py). When authed + exclude
is on, we read active follows the same way E2/E3 do (public_property_followers export, active = deleted_at
IS NULL). If the source is missing/unreadable, we LOG and degrade to no-exclusion rather than fail search.
"""
...
class FollowGate:
    ...
    def followed(self, user_id: Optional[int], exclude_followed: bool) -> Tuple[Set[int], dict]:
        ...
        try:
            ...
            return set(ids), {"applied": True, "source": self._backend, "count": len(ids)}
