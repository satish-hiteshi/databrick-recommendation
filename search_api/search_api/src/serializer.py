...
def serialize_result(r, query: str, followed: Set[int], debug: bool) -> dict:
    name_path = r.match_type in ("exact", "fuzzy")
    return {
        "type": "property",
        ...
        "match_type": "exact" if name_path else "thematic",     # schema allows exact|thematic
        "disambiguation_confidence": round(r.disambiguation_confidence, 4),
        "why_string": (WHY_NAME if name_path else WHY_THEMATIC).format(q=query),
        "badge": None,
        "is_followed": r.property_id in followed,
        "follow_cta": True,
        "latest_moment": None,                                  # best-effort context; null at this stage
        "debug": ({"match_type_raw": r.match_type, "cosine": r.cosine, "signals": r.signals,
                   "centrality_pct": round(r.centrality_pct, 4),
